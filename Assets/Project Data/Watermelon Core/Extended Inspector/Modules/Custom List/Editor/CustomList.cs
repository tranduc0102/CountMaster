...
namespace Watermelon.List
{
    public class CustomList
    {
        ...
        private void HandleDraggingDetection()
        {
            if (!dragging)
            {
                if ((currentEvent.type == EventType.MouseDrag) && globalRect.Contains(currentEvent.mousePosition) && (currentEvent.delta.magnitude < 5f) &&  Mathf.Approximately((lastMouseDownPosition - currentEvent.mousePosition).magnitude, 1f))// 5f is very arbitrary number and this condition is here to fix bug with Object field
                {
                    DraggingStarted();
                }
            }
